...
Install the dependencies. You'll also need `poetry` for managing Python dependencies. The dependencies are pretty simple so you can probably replace `poetry` with something else if you have philosophical objections to one dependency manager over another.
```
make env
```
